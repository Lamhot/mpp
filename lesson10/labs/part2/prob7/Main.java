package lesson10.labs.part2.prob7;
...
public class Main {
	...
	public static void ordering1(List<Integer> list) {
		System.out.println(
				list.stream().sorted(Comparator.comparing(x -> Math.abs((Integer) x)).thenComparing(y -> (Integer) y))
	...
	public static void ordering2(List<String> words) {
		System.out.println(words.stream().sorted(Comparator.comparing(x -> new StringBuilder(x).reverse().toString()))
				.collect(Collectors.toList()));
