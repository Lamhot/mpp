...
//		test5();
	...
	public static <T> boolean contains3(List<T> list, T ob, BiPredicate<? super T, ? super T> eq) {
		for (T x : list) {
			if (eq.test(x, ob)) {
				return true;
			}
	...
	public static <T> boolean contains4(List<? extends T> list, T ob, BiPredicate<? super T, ? super T> eq) {
		for (T x : list) {
			if (eq.test(x, ob)) {
				return true;
			}
	...
	public static <T> boolean contains5(List<? extends T> list, T on, BiPredicate<? super T, ? super T> eq) {
		for (T x : list) {
			if (eq.test(x, on))
				return true;
		}
	...
	public static void test6() {
		...
		boolean foundIt = Main.contains4(list, emp, (Employee e, Person p) -> e.getName().equals(p.getName()));
		System.out.println(foundIt);
	}
